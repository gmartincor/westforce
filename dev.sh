...
if [ "$MODE" = "landing" ]; then
    ...
    echo ""
    DEV_FORCE_LANDING=True docker-compose run --rm -p $PORT:$PORT web python manage.py runserver 0.0.0.0:$PORT
elif [ "$MODE" = "manager" ]; then
    echo "🎛️  MANAGER PANEL MODE"
    echo "Access: http://localhost:$PORT/"
    echo "Login: westforce / westforce"
    echo ""
    DEV_FORCE_LANDING=False docker-compose run --rm -p $PORT:$PORT web python manage.py runserver 0.0.0.0:$PORT
else
